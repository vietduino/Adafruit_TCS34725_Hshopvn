/*!
 *  @file Adafruit_TCS34725_Hshopvn.cpp
 *
 ...
 *  @param  value
 */
void Adafruit_TCS34725_Hshopvn::write8(uint8_t reg, uint32_t value) {
 ...
 *  @return value
 */
uint8_t Adafruit_TCS34725_Hshopvn::read8(uint8_t reg) {
  _wire->beginTransmission(_i2caddr);
#if ARDUINO >= 100
  _wire->write(TCS34725_COMMAND_BIT | reg);
#else
 ...
 *  @return value
 */
uint16_t Adafruit_TCS34725_Hshopvn::read16(uint8_t reg) {
  uint16_t x;
  uint16_t t;
 ...
 */
void Adafruit_TCS34725_Hshopvn::enable() {
 ...
 */
void Adafruit_TCS34725_Hshopvn::disable() {
 ...
 *          Gain
 */
Adafruit_TCS34725_Hshopvn::Adafruit_TCS34725_Hshopvn(tcs34725IntegrationTime_t it,
                                     tcs34725Gain_t gain) {
 ...
 */
boolean Adafruit_TCS34725_Hshopvn::begin(uint8_t addr) {
  _i2caddr = addr;
  _wire = &Wire;
 ...
 *  @return True if initialization was successful, otherwise false.
 */
boolean Adafruit_TCS34725_Hshopvn::begin(uint8_t addr, TwoWire *theWire) {
  _i2caddr = addr;
  _wire = theWire;
 ...
 *  @return True if initialization was successful, otherwise false.
 */
boolean Adafruit_TCS34725_Hshopvn::begin() {
 ...
 *  @return True if initialization was successful, otherwise false.
 */
boolean Adafruit_TCS34725_Hshopvn::init() {
  _wire->begin();
 ...
 *          Integration Time
 */
void Adafruit_TCS34725_Hshopvn::setIntegrationTime(tcs34725IntegrationTime_t it) {
 ...
 */
void Adafruit_TCS34725_Hshopvn::setGain(tcs34725Gain_t gain) {
  if (!_tcs34725Initialised)
    begin();
 ...
 *          Clear channel value
 */
void Adafruit_TCS34725_Hshopvn::getRawData(uint16_t *r, uint16_t *g, uint16_t *b,
                                   uint16_t *c) {
  if (!_tcs34725Initialised)
    begin();
 ...
 *          Clear channel value
 */
void Adafruit_TCS34725_Hshopvn::getRawDataOneShot(uint16_t *r, uint16_t *g, uint16_t *b,
                                          uint16_t *c) {
  if (!_tcs34725Initialised)
    begin();
 ...
 */
void Adafruit_TCS34725_Hshopvn::getRGB(float *r, float *g, float *b) {
 ...
 */
uint16_t Adafruit_TCS34725_Hshopvn::calculateColorTemperature(uint16_t r, uint16_t g,
                                                      uint16_t b) {
 ...
 *  @return Color temperature in degrees Kelvin
 */
uint16_t Adafruit_TCS34725_Hshopvn::calculateColorTemperature_dn40(uint16_t r,
                                                           uint16_t g,
                                                           uint16_t b,
                                                           uint16_t c) {
 ...
 *  @return Lux value
 */
uint16_t Adafruit_TCS34725_Hshopvn::calculateLux(uint16_t r, uint16_t g, uint16_t b) {
  float illuminance;
 ...
 */
void Adafruit_TCS34725_Hshopvn::setInterrupt(boolean i) {
 ...
 */
void Adafruit_TCS34725_Hshopvn::clearInterrupt() {
  _wire->beginTransmission(_i2caddr);
#if ARDUINO >= 100
 ...
 *          High limit
 */
void Adafruit_TCS34725_Hshopvn::setIntLimits(uint16_t low, uint16_t high) {
